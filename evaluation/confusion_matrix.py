...
test = pd.read_csv('data/record/test_labels.csv')
pred = pd.read_csv('data/record/test_pred.csv')
test['filename'] = test['filename'].apply(lambda x : x.split('/')[-1])
pred['filename'] = pred['filename'].apply(lambda x : x.split('/')[-1])
label_map = "data/record/label_map.txt"
output_path = "data/record/confusion_matrix.csv"
